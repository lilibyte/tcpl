...
// i dont like this question because i dont need to push back
// an entire string onto the input. ungetch is only ever called
// for one character at a time so i dont even know how to
// test this or know i'm accomplishing the point of the
// exercise

// i didnt use ungetch() so the remaining space can just be
// calculated once and the function can be abandoned if there
// won't be enough space in buf (len + bufp >= BUFSIZE)
...
/* ungets:  push charaters back on input stack */
void ungets(char s[]) {
  int len = strlen(s);
  if (len + bufp >= BUFSIZE)
